...
## 37. Classification with Density Estimation: The Complete Rule

**Front:** When using methods like Parzen Window or K-NN for classification, do we simply assign a test point to the class with the highest estimated density $p(x|C_k)$ at that point?
**Back:**
**No.** That would ignore class prior probabilities. The correct, optimal rule (Bayes classifier) is to assign the point to the class with the highest **posterior probability**, which combines both density **and** prior:

1. Estimate the **class-conditional density** $\hat{p}(x|C_k)$ for each class (e.g., using Parzen or K-NN density estimation on that class's data).
2. Estimate the **class prior** $\hat{P}(C_k)$ (e.g., as the fraction of training data in class $k$).
3. Compute the (unnormalized) posterior: $\hat{P}(C_k|x) \propto \hat{p}(x|C_k) \cdot \hat{P}(C_k)$.
4. Assign to class $j^* = \arg\max_k \hat{P}(C_k|x)$.
